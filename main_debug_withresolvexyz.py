...
def dectshow(org_img, class_ids, boxs, depth_data, depth_intrin):
    img = org_img.copy()
    if len(boxs)>0:
        for box, text_class in zip(boxs, class_ids):
            ...
            if text_class == 'bowl':
                ...
                depth_point = rs.rs2_deproject_pixel_to_point(depth_intrin, [y, x], dist)
                print('xyz = ' + str(depth_point[0]) + ' ,'+ str(depth_point[1]) + ' ,' + str(depth_point[2]))
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            dectshow(color_image, class_ids, boxs, depth_image, depth_intrin)
